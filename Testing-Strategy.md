...
**Related:** [Verification and Review](Verification-and-Review.md) · [Integration Contracts](Integration-Contracts.md) · [Agentic Workflow Guide](agentic-workflow-guide.md)
...
> *See: [Verification and Review](Verification-and-Review.md) for how testing maps to the broader feedback loop pattern.*
...
## E2E Smoke Testing in the Agentic Workflow

The agentic workflow guide includes the `/e2e-smoke-run` subagent, which runs lightweight Playwright smoke tests generated by `/e2e-smoke-plan`. This mechanism catches integration boundary bugs that unit tests miss — situations where code compiles and passes in isolation but breaks at the UI layer or across service boundaries.

Smoke tests complement the full E2E suite by being faster and more focused. They exercise critical user flows (login → dashboard → action → logout) rather than exhaustive coverage. They catch regressions in direct UI changes and downstream backend changes. For the complete E2E smoke pipeline, including how to generate and run smoke test plans from feature specifications, see [Agentic Workflow Guide](agentic-workflow-guide.md).
