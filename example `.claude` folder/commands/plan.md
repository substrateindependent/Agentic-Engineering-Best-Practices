You are creating an implementation plan from a diagnosis. Your job is to turn the diagnosis into a concrete, ordered plan file with specific files, changes, and acceptance criteria.

**Ask the user**: "Paste the diagnosis block from `/diagnose`."

Once the user pastes the diagnosis, do the following:

**Step 1 — Parse the diagnosis.** Extract:
...
- Each piece should have a clear and detailed "Done when" that an agent can verify. This is most important aspect of the plan. A properly scoped "Done when" serves as the acceptance criteria for a piece and must be designed carefully to ensure that satisfying the "Done when" scope will result in the intended behavior or functionality being successfully delivered and integrated into the codebase. Scope "Done when" to ensure code is not only functional but also integrated to prevent orphaned code.
- For pieces with `E2E scope: downstream`, the "Done when" should include both the direct technical criterion AND the expected downstream user-visible effect. For example: "Briefing generation uses UTC midnight boundary; unit tests confirm correct time window. The /admin/briefing page should show data aggregated from the correct 24-hour window." This gives the E2E smoke planner concrete assertions to verify through the browser.
...
**Step 5 — Coverage check.** Before presenting the plan, map every item from the diagnosis's "What's Broken or Missing" list to a plan piece. If any diagnosis item is not covered by a piece, either add a piece for it or list it explicitly as "Deferred" with a reason at the bottom of the plan (after the Current State section). No silent omissions — every diagnosis item must be accounted for. Also verify that every piece with `E2E scope: downstream` or `direct` has a corresponding entry in the "E2E Verification Plan" section. If a piece claims downstream visibility but no verification flow is listed, either add one or downgrade the scope to `none` with justification.

**Step 6 — Present the plan to the user.** After writing the file, show them the plan contents.

**Step 7 — Plain-language summary and quality review.** After presenting the plan, include two additional sections in your response (not in the plan file itself):

1. **"What we're doing" summary** — Explain the plan in simple, intuitive, non-technical terms. Assume the reader is not a developer. Describe what each piece accomplishes in terms of the user-facing or system-level outcome, not the code mechanics. For example, say "We're making sure the calendar shows the right time zone" instead of "We're refactoring the DateTime serialization in the API response handler."

2. **"How we'll know it worked" review** — Address three questions:
   - **Verification**: How will we test that the plan's goals were actually achieved and the features work as intended? Are the "Done when" criteria sufficient, or should any be strengthened?
   - **Downstream safety**: What could this plan break in adjacent code? Are there integration points, consumers, or dependencies that need protection?
   - **Code hygiene**: Does this plan risk introducing orphaned code, dead imports, tech debt, or unnecessary complexity? If so, which pieces should include cleanup steps?
   - **E2E coverage:** Does the E2E Verification Plan adequately cover the user-visible effects of this plan? Are there backend changes that could break the UI but aren't being verified through the browser? Should any `E2E scope: none` pieces be reconsidered?

   If this review surfaces improvements to the plan (stronger acceptance criteria, missing test pieces, cleanup steps), offer to update the plan file before the user proceeds to `/implement`.

Finally, say: "Review this plan: <LINK PATH>. Edit anything you'd change, then run `/implement` in a fresh session pointing to this plan file."

Do NOT implement any code. This command only produces the plan document.
