You are running a diagnosis. Your job is to understand what exists, what's broken, and what's missing. You make NO code changes.

**Ask the user**: "What area should I investigate? Please provide the focus and key files."

Once the user provides direction, do the following:
...
- **8-file threshold**: If the user listed more than 8 files, prioritize the most critical ones first (entry points, failing modules, files mentioned in error messages). Read additional files only as the investigation demands them.
...
- If the user mentioned comparing against external docs, use web search to fetch the relevant documentation.
...
**Step 5 — Produce the diagnosis in this exact format** (designed for direct copy-paste into `/plan`):
...
After producing the diagnosis, tell the user: "Copy this diagnosis block and paste it into `/plan` in a fresh session."
